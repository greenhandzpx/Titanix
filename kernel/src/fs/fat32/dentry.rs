...
use super::{
    util::shortname_checksum,
    SNAME_LEN, LNAME_MAXLEN,
    time::FAT32Timestamp, file::FAT32File,
};
...
impl FAT32DirEntry {
    pub fn fname(&self) -> String {
        ...
        if lname_len > 0 {
            ...
            if ext_len > 0 {
                String::from_utf8_lossy(&base).to_string() + "." + &String::from_utf8_lossy(&ext).to_string()
            } else {
    ...
    fn write_dentry(&self, writer: &mut dyn DentryWriter) {
        ...
        macro_rules! s_lname {
            ($data_idx: expr, $lname_idx: expr) => {
                lsb16!($data_idx, {
                    if $lname_idx == lname_len { 0 }
                    else if $lname_idx > lname_len { 0xFFFF }
                    else { self.lname[$lname_idx] }
                });
            };
        }
