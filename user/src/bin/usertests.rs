...
static SUCC_TESTS: &[(&str, &str, &str, &str, i8)] = &[
    ("exit\0", "\0", "\0", "\0", 0),
...
static FAIL_TESTS: &[(&str, &str, &str, &str, i8)] = &[("stack_overflow\0", "\0", "\0", "\0", -2)];

use user_lib::{execve, fork, waitpid};

fn run_tests(tests: &[(&str, &str, &str, &str, i8)]) -> i32 {
    let mut pass_num = 0;
    ...
    for test in tests {
        ...
        if pid == 0 {
            execve(
                test.0,
                &[(*test.0).as_ptr(), core::ptr::null::<u8>()],
                &[core::ptr::null::<u8>()],
            );
            panic!("unreachable!");
        } else {
            ...
            if wexitstatus!(exit_code) as i8 == test.4 {
